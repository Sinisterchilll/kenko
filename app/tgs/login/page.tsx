...
export default function TGSLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/auth/tgs/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, password }),
      });
      if (res.ok) {
        router.push("/tgs/dashboard");
      } else {
      ...
      <div style={{
        ...
        <div style={{ marginTop: "auto", marginBottom: "auto", display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          <p style={{ color: "var(--text-dim)", maxWidth: 440, fontSize: 15, lineHeight: 1.6 }}>
            Real-time visibility into every order that moves through a Bounce hub —
            inflow, in-transit, delivered, and everything in between.
          </p>
          ...
          <div style={{
            ...
            {[
              { num: "1", label: "Active hub" },
              { num: "Live", label: "Order tracking" },
              { num: "POD", label: "Delivery proof" },
            ].map((s, i) => (
        ...
        >
          <div>
            ...
            <p style={{ color: "var(--text-mute)", fontSize: 13, marginTop: 8 }}>
              Use your TGF work email to access the hub dashboard.
            </p>
          </div>
